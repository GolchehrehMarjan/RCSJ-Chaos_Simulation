...
[II,JJ] = ndgrid(1:length(Iac_grid),1:length(Idc_grid));
nTotal   = numel(II);
lyap_vec = zeros(nTotal,1);

tic;
parfor kk = 1:nTotal
    lyap_vec(kk) = lyapunov_exponent( ...
        Betha,Idc_grid(JJ(kk)),Iac_grid(II(kk)), ...
        Omega,T,n_periods,discard,d0,opts_lyap);
end
fprintf('Grid computation done | elapsed = %.1f min\n',toc/60);

lyap_map = reshape(lyap_vec,length(Iac_grid),length(Idc_grid));
